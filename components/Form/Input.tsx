...
const Input = React.forwardRef<InputRef & TextAreaRef, InputI>((props, ref) => {
  const { type, label, className, message, messageType, icon, ...inputProps } = props;
      ...
      >
        {type === "textarea" ? (
          ...
          <input
            ...
            ref={ref}
            type={inputType}
            className={`w-full h-full rounded-2xl p-4 ${icon === "price" ? "pl-8" : ""} ${className || ""}`}
          />
        )}
        {type === "password" && (
        ...
        {icon === "discount" && <span className="absolute top-3 right-4">%</span>}
      </div>
